...
PlannerServer::on_configure(const rclcpp_lifecycle::State& state)
{
  ...
  RCLCPP_INFO(get_logger(),
              "Planner Server has %s planners available.",
...
template<typename T>
bool
PlannerServer::isServerInactive(
...
template<typename T>
bool
PlannerServer::isCancelRequested(
  std::unique_ptr<nav2_util::SimpleActionServer<T>>& action_server)
{
...
template<typename T>
void
PlannerServer::getPreemptedGoalIfRequested(
  std::unique_ptr<nav2_util::SimpleActionServer<T>>& action_server,
...
template<typename T>
bool
PlannerServer::transformPosesToGlobalFrame(
  std::unique_ptr<nav2_util::SimpleActionServer<T>>& action_server,
...
PlannerServer::computePlanThroughPoses()
{
  ...
  try {
    ...
    for (unsigned int i = 0; i != goal->goals.size(); i++) {
      ...
      if (!transformPosesToGlobalFrame(
            action_server_poses_, curr_start, curr_goal)) {
        return;
      }
      ...
      if (!validatePath(
            action_server_poses_, curr_goal, curr_path, goal->planner_id)) {
        return;
      }

      // Concatenate paths together
      concat_path.poses.insert(concat_path.poses.end(),
                               curr_path.poses.begin(),
                               curr_path.poses.end());
    ...
    RCLCPP_WARN(get_logger(),
                ...
                goal->goals.back().pose.position.y,
                ex.what());
    action_server_poses_->terminate_current();
  }
}
...
PlannerServer::computePlan()
{
  ...
  try {
    ...
    if (!validatePath(
          action_server_pose_, goal_pose, result->path, goal->planner_id)) {
      return;
    }
...
nav_msgs::msg::Path
PlannerServer::getPlan(const geometry_msgs::msg::PoseStamped& start,
                       const geometry_msgs::msg::PoseStamped& goal,
                       const std::string& planner_id)
{
  RCLCPP_DEBUG(get_logger(),
...
PlannerServer::dynamicParametersCallback(
